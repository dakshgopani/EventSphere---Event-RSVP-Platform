...
function getEventBanner(eventId, bannerUrl) {
  if (bannerUrl) return bannerUrl;

  // Deterministic hash → same event gets same banner
  let hash = 0;
  for (let i = 0; i < eventId.length; i++) {
    hash = eventId.charCodeAt(i) + ((hash << 5) - hash);
  }

  const index = Math.abs(hash) % EVENT_BANNERS.length;
  return EVENT_BANNERS[index];
}
...
function displayEvents(events) {
    ...
    grid.innerHTML = events.map((event, index) => `
        <div class="event-card" onclick="openEventModal('${event.event_id}')">
            <div class="event-banner" style="background-image: url('${event.banner_url || EVENT_BANNERS[index % EVENT_BANNERS.length]}')"
...
function getBannerByIndex(eventId) {
  const cards = Array.from(document.querySelectorAll(".event-card"));
  const index = cards.findIndex(card =>
    card.getAttribute("onclick").includes(eventId)
  );
  return EVENT_BANNERS[index % EVENT_BANNERS.length];
}
...
async function openEventModal(eventId) {
  try {
    ...
    const banner = event.banner_url || getBannerByIndex(event.event_id);
